...
void ds50::V172xFragment::checkADCData(int daq_adc_bits) const {
  ds50::V172xFragment::adc_type const * adcPtr(findBadADC(daq_adc_bits));
  if (adcPtr != dataEnd()) {
...
bool ds50::V172xFragment::channel_present(int ch) const { 
  if (ch < 0 || ch > 7) throw cet::exception("IllegalChannel") << "requested ch #" << ch << ",";
  return header_()->channel_mask && (1 << ch); 
}

unsigned int ds50::V172xFragment::enabled_channels() const {
  return pop_count (channel_mask ());
}

ds50::V172xFragment::adc_type const * ds50::V172xFragment::chDataBegin(int ch) const {
...
std::ostream & ds50::operator << (std::ostream & os, V172xFragment const & b) {
  os << "V172xFragment ID: "
